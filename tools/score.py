...
WARNING: the music has 8 voices of its own in vg_synth.cpp. The sound effects
have a different pool of 10, and neither pool can take a voice from the other.
Above 8 voices the music takes the voice with the least time left from itself.
Keep the peak voice count in the report at 8 or less.
"""
import argparse
import os
import re
...
def report(s, notes):
    ...
    print("")
    print("voices     peak %d of 8 at once" % peak)
    if peak > 8:
        print("  WARNING: the music has 8 voices. Above 8 a note takes the voice with")
        print("  the least time left from another note, and that note stops early.")
...
def bake(s, notes, spec_path):
    ...
    out.append("// GENERATED by tools/score.py from %s -- do not edit."
               % rel(spec_path))
    out.append("//")
    out.append("// %d notes, %.1f seconds, peak %d of 8 music voices."
