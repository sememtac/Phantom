...
// lines that shared nothing with what surrounds them, and its one call site in draw_band is
// behind vg_tv_active(), so the frame reaches it only during a transition.
// ---------------------------------------------------------------------------
...
// ROW_SPLIT moved to vg_raster_int.h: the canopy needs it too, and it is the row
// split's constant rather than this file's.
...
// ===========================================================================
// THE CANOPY MOVED to vg_canopy_draw.cpp. 1,112 lines: the drawing, its colour table,
// the warp maps, the coming-online sequence and the row pass over all of it.
//
// It went WHOLE, and that is why it publishes almost nothing. Lifting the intro alone
// would have forced s_can_lut, canopy_lut(), s_can_ready and s_intro_on into public
// view to move 311 lines; the whole system needs three functions and keeps the rest.
//
// The row split already dispatched across a translation unit to vg_sky_fill_rows, so a
// canopy on the other side of one is the same shape as the backdrop and costs the same.
// ===========================================================================
