...
static inline float plane_d(void) {
    return -vdot(vg.course.ring_pos, vg.course.ring_norm);
}
...
static void place_ring(void) {
    ...
    const float margin = ((vg.course.index % COURSE_NEAR_WALL) == 0)
                       ? (ARENA_ALERT_RANGE * 0.55f)
                       : ARENA_SPAWN_MARGIN;
    pos = vg_arena_clamp_inside(pos, margin);

    vg.course.ring_pos  = pos;
    // Facing back down the approach, so the gate presents as a circle rather than
    // an edge-on line when it first appears.
    vg.course.ring_norm = vnorm(pos);
    vg.course.ring_alive = true;
    vg.course.ring_prev_d = plane_d();
}

void vg_course_begin(void) {
    vg.course.hits  = 0;
    vg.course.index = 0;
    vg.course.done  = false;
    vg.course.end_t = 0.0f;
    ...
    vg.course.briefing = true;
    vg.course.named    = false;
    vg.course.wait     = COURSE_SETTLE;
...
void vg_course_reset_streak(void) {
    ...
    if (vg.course.briefing) { vg.course.ring_alive = false; return; }
    place_ring();
}
...
void vg_course_update(float dt) {
    ...
    if (!vg.course.ring_alive) {
        ...
        if (vg.course.briefing) {
            ...
            // by a different door.
            if (vg_ift_busy() || vg.course.greet > 0.0f) {
                vg.course.wait = COURSE_SETTLE;
                return;
            }
            vg.course.wait -= dt;
            if (vg.course.wait > 0.0f) return;
            vg.course.briefing = false;
            vg.course.named    = true;   // however the briefing ended
        }
        place_ring();
        return;
    }
    ...
    if (vlen2(vg.course.ring_pos) > COURSE_LOST_DIST * COURSE_LOST_DIST) {
        place_ring();
        return;
    }

    if (vg.course.ring_prev_d < 0.0f && d >= 0.0f) {
        ...
        const Vec3  p     = vmul(vg.course.ring_pos, -1.0f);       // player, gate-relative
        const Vec3  inpl  = vsub(p, vmul(vg.course.ring_norm, vdot(p, vg.course.ring_norm)));
        const float miss  = vlen(inpl);

        if (miss <= COURSE_RADIUS) {
            vg.course.hits++;
            if (vg.course.hits >= COURSE_TARGET) {
                vg.course.done = true;
                vg.course.ring_alive  = false;
...
void vg_course_draw(const VgCam& cam) {
    if (!vg.course.ring_alive) return;

    // Any two axes perpendicular to the normal will do; there is no preferred
    // roll for a circle.
    Vec3 a = vcross(vg.course.ring_norm, v3(0, 1, 0));
    if (vlen2(a) < 1e-4f) a = vcross(vg.course.ring_norm, v3(1, 0, 0));
    a = vnorm(a);
    const Vec3 b = vnorm(vcross(vg.course.ring_norm, a));

    const float TAU = 6.28318531f;
    Vec3 prev = vadd(vg.course.ring_pos, vmul(a, COURSE_RADIUS));
    ...
    for (int i = 1; i <= COURSE_SEGS; i++) {
        ...
        const Vec3  cur = vadd(vg.course.ring_pos,
