...
// The reticle is GONE, replaced by the canopy's aperture -- see THE CANOPY below.
...
// ---------------------------------------------------------------------------
// THE CANOPY IS GONE FROM HERE. The ink versions lived in this file -- an
// aperture with struts in vg_line, then lit tapering members in vg_tri -- and
// the shipped cockpit is the author's baked drawing, applied as light by
// vg_canopy_draw.cpp. The aperture still does the reticle's job there: it
// brackets where the nose points without drawing anything on it. The ink
// versions' measured dead ends moved with them, to the note at THE BAKED
// CANOPY in that file.
// ---------------------------------------------------------------------------
...
void vg_draw_hud(const VgCam& cam, const VgInput* in, float fps) {
    ...
    // Nothing else in this corner. The frame counter lives in vg_render's
    ...
    if (vg.state == VG_COURSE) {
        char cbuf[16];
        snprintf(cbuf, sizeof(cbuf), "%d/%d", (int)vg_course.hits, COURSE_TARGET);
